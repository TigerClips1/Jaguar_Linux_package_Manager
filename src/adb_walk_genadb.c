...
static int adb_walk_genadb_schema(struct adb_walk *d, uint32_t schema_id)
{
	...
	if (!s || !s->magic) return -PS4E_ADB_SCHEMA;

	adb_wo_init(&dt->objs[0], &dt->vals[0], s->root, &dt->db);
	dt->num_vals += s->root->num_fields;
	if (dt->num_vals >= ARRAY_SIZE(dt->vals)) return -PS4E_ADB_LIMIT;
	dt->nest = 0;

	return 0;
}

static int adb_walk_genadb_comment(struct adb_walk *d, ps4_blob_t comment)
{
	return 0;
}
...
static int adb_walk_genadb_start_object(struct adb_walk *d)
{
	...
	if (!dt->db.schema) return -PS4E_ADB_SCHEMA;
	if (dt->nest >= ARRAY_SIZE(dt->objs)) return -PS4E_ADB_LIMIT;

	if (dt->curkey[dt->nest] == 0 &&
	    dt->objs[dt->nest].schema->kind == ADB_KIND_OBJECT)
		return -PS4E_ADB_SCHEMA;
	...
	if (*adb_ro_kind(&dt->objs[dt->nest-1], dt->curkey[dt->nest-1]) == ADB_KIND_ADB) {
		...
		if (dt->nestdb >= ARRAY_SIZE(dt->idb)) return -PS4E_ADB_LIMIT;
...
static int adb_walk_genadb_key(struct adb_walk *d, ps4_blob_t key)
{
	struct adb_walk_genadb *dt = container_of(d, struct adb_walk_genadb, d);
...
static int adb_walk_genadb_scalar(struct adb_walk *d, ps4_blob_t scalar, int multiline)
{
	struct adb_walk_genadb *dt = container_of(d, struct adb_walk_genadb, d);
