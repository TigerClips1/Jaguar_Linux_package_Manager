...
struct conv_script {
	...
	char csum_len;
	char csum[2*PS4_DIGEST_LENGTH_MAX];
	int type;
	size_t size;
	ps4_blob_t *triggers;
	char script[];
...
static int read_script(void *pctx, const struct ps4_file_info *ae, struct ps4_istream *is)
{
	...
	s->type = type;
	s->size = ae->size;
	ps4_istream_read(is, s->script, s->size);
...
static struct conv_script *find_pkg(struct conv_ctx *ctx, ps4_blob_t identity, int type)
{
	struct conv_script *s;
	list_for_each_entry(s, &ctx->script_head, script_node)
		if (ps4_blob_compare(PS4_BLOB_PTR_LEN(s->csum, s->csum_len), identity) == 0)
			return s;
	return 0;
}

static int read_triggers(struct conv_ctx *ctx, struct ps4_istream *is)
{
	ps4_blob_t l, r, nl = PS4_BLOB_STR("\n"), spc = PS4_BLOB_STR(" ");
	struct conv_script *s;

	if (IS_ERR(is)) return PTR_ERR(is);

	while (ps4_istream_get_delim(is, nl, &l) == 0) {
		if (!ps4_blob_split(l, spc, &l, &r)) continue;
		s = find_pkg(ctx, l, ADBI_SCRPT_TRIGGER);
		if (!s) continue;

		s->triggers = ps4_atomize_dup(&ctx->atoms, r);
	}
	return ps4_istream_close(is);
}

static int convert_idb(struct conv_ctx *ctx, struct ps4_istream *is)
{
	struct ps4_id_cache *idc = ps4_ctx_get_id_cache(ctx->ac);
	struct ps4_digest digest;
	struct adb_obj pkg, pkginfo, files, file, paths, path, scripts, triggers, acl;
	ps4_blob_t l, val, spc = PS4_BLOB_STR(" "), nl = PS4_BLOB_STR("\n");
	struct conv_script *s;
	int i;
	...
	while (ps4_istream_get_delim(is, nl, &l) == 0) {
		...
		val = PS4_BLOB_PTR_LEN(l.ptr+2, l.len-2);
		...
		case 'C': // pkg checksum
			list_for_each_entry(s, &ctx->script_head, script_node) {
				if (ps4_blob_compare(PS4_BLOB_PTR_LEN(s->csum, s->csum_len), val) != 0)
					continue;

				adb_wo_blob(&scripts, s->type, PS4_BLOB_PTR_LEN(s->script, s->size));
				if (s->type == ADBI_SCRPT_TRIGGER && s->triggers) {
					ps4_blob_t r = *s->triggers, l = *s->triggers;
					while (ps4_blob_split(r, spc, &l, &r))
		...
		case 'M': // directory mode: uid:gid:mode:xattrcsum
			adb_wo_blob(&acl, ADBI_ACL_USER, ps4_id_cache_resolve_user(idc, ps4_blob_pull_uint(&val, 10)));
			ps4_blob_pull_char(&val, ':');
			adb_wo_blob(&acl, ADBI_ACL_GROUP, ps4_id_cache_resolve_group(idc, ps4_blob_pull_uint(&val, 10)));
			ps4_blob_pull_char(&val, ':');
			adb_wo_int(&acl, ADBI_ACL_MODE, ps4_blob_pull_uint(&val, 8));
		...
		case 'Z': // file content hash
			...
			adb_wo_blob(&file, ADBI_FI_HASHES, PS4_DIGEST_BLOB(digest));
			break;
...
static int conv_main(void *pctx, struct ps4_ctx *ac, struct ps4_string_array *args)
{
	...
	struct adb_obj idb;
	int r;
	int root_fd = ps4_ctx_fd_root(ac);

	ctx->ac = ac;
	list_init(&ctx->script_head);
	ps4_atom_init(&ctx->atoms);
	...
	ps4_tar_parse(
		ps4_istream_from_file(root_fd, "lib/ps4/db/scripts.tar"),
		read_script, ctx, ps4_ctx_get_id_cache(ac));

	read_triggers(ctx, ps4_istream_from_file(root_fd, "lib/ps4/db/triggers"));

	convert_idb(ctx, ps4_istream_from_file(root_fd, "lib/ps4/db/installed"));

	adb_wo_obj(&idb, ADBI_IDB_PACKAGES, &ctx->pkgs);
	adb_w_rootobj(&idb);

	r = adb_c_create(
		//ps4_ostream_to_file(db->root_fd, "lib/ps4/db/installed.adb", 0644),
		adb_compress(ps4_ostream_to_file(AT_FDCWD, "installed.adb", 0644), &ac->compspec),
		&ctx->dbi, ps4_ctx_get_trust(ac));
	if (r == 0) {
		// unlink old files
	}
	...
	adb_free(&ctx->dbi);
	adb_free(&ctx->dbp);
	ps4_atom_free(&ctx->atoms);

	return r;
}

static struct ps4_applet ps4_convdb = {
	.name = "convdb",
	.context_size = sizeof(struct conv_ctx),
	.optgroups = { &optgroup_global, &optgroup_generation },
	.main = conv_main,
};
PS4_DEFINE_APPLET(ps4_convdb);
