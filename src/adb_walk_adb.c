...
static int adb_walk_block(struct adb *db, struct adb_block *b, struct ps4_istream *is);
...
static int dump_item(struct adb_walk_ctx *ctx, const char *name, const uint8_t *kind, adb_val_t v)
{
	...
	d->ops->key(d, name ? PS4_BLOB_STR(name) : PS4_BLOB_NULL);
	...
	case ADB_KIND_ARRAY:
		...
		break;
	case ADB_KIND_ADB:
		ps4_istream_from_blob(&is, adb_r_blob(&ctx->db, v));
		origdb = ctx->db;
...
static int adb_walk_block(struct adb *db, struct adb_block *b, struct ps4_istream *is)
{
	...
	ps4_blob_t data, c = PS4_BLOB_BUF(tmp);
	int r;
	...
	case ADB_BLOCK_ADB:
		...
		hdr = ps4_istream_peek(is, sizeof *hdr);
		if (IS_ERR(hdr)) return PTR_ERR(hdr);
		ps4_blob_push_fmt(&c, "ADB block, size: %" PRIu64 ", compat: %d, ver: %d",
			sz, hdr->adb_compat_ver, hdr->adb_ver);
		d->ops->comment(d, ps4_blob_pushed(PS4_BLOB_BUF(tmp), c));
		if (ds->root && hdr->adb_compat_ver == 0) dump_object(ctx, ds->root, adb_r_root(db));
		return 0;
	case ADB_BLOCK_SIG:
		s = (struct adb_sign_hdr*) ps4_istream_get(is, sz);
		data = PS4_BLOB_PTR_LEN((char*)s, sz);
		r = adb_trust_verify_signature(ctx->trust, db, &ctx->vfy, data);
		ps4_blob_push_fmt(&c, "sig v%02x h%02x ", s->sign_ver, s->hash_alg);
		for (size_t j = sizeof *s; j < data.len && c.len > 40; j++)
			ps4_blob_push_fmt(&c, "%02x", (uint8_t)data.ptr[j]);
		if (c.len <= 40) ps4_blob_push_blob(&c, PS4_BLOB_STRLIT(".."));
		ps4_blob_push_fmt(&c, ": %s", r ? ps4_error_str(r) : "OK");
		break;
	case ADB_BLOCK_DATA:
		ps4_blob_push_fmt(&c, "data block, size: %" PRIu64, sz);
		break;
	default:
		ps4_blob_push_fmt(&c, "unknown block %d, size: %" PRIu64, adb_block_type(b), sz);
		break;
	}
	d->ops->comment(d, ps4_blob_pushed(PS4_BLOB_BUF(tmp), c));
	return 0;
...
{
	struct ps4_trust allow_untrusted = {
		.allow_untrusted = 1,
	};
